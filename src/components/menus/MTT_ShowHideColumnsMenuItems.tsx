import classes from "./MTT_ShowHideColumnsMenuItems.module.css";
...
import { MTT_ColumnPinningButtons } from "../buttons/MTT_ColumnPinningButtons";
...
export const MTT_ShowHideColumnsMenuItems = <TData extends MTT_RowData>({
  allColumns,
  column,
  hoveredColumn,
  setHoveredColumn,
  ...
  const handleToggleColumnHidden = (column: MTT_Column<TData>) => {
    if (columnDefType === "group") {
      column?.columns?.forEach?.((childColumn: MTT_Column<TData>) => {
    ...
    <>
      ...
      >
        <Box className={classes.menu}>
          ...
          {enableColumnPinning &&
            (column.getCanPin() ? (
              <MTT_ColumnPinningButtons column={column} table={table} />
            ) : (
      ...
      {column.columns?.map((c: MTT_Column<TData>, i) => (
        <MTT_ShowHideColumnsMenuItems
