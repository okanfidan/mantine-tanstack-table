...
import classes from "./MTT_ToolbarAlertBanner.module.css";
...
import { getMTT_SelectAllHandler } from "../../utils/row.utils";
import { parseFromValuesOrFunc } from "../../utils/utils";
import { MTT_SelectCheckbox } from "../inputs/MTT_SelectCheckbox";

interface Props<TData extends MTT_RowData> extends Partial<AlertProps> {
  stackAlertBanner?: boolean;
  table: MTT_TableInstance<TData>;
}

export const MTT_ToolbarAlertBanner = <TData extends MTT_RowData>({
  stackAlertBanner,
  table,
  ...rest
}: Props<TData>) => {
  ...
  const selectedAlert = selectedRowCount ? (
    <Flex align="center" gap="sm">
      ...
      <Button
        onClick={(event) =>
          getMTT_SelectAllHandler({ table })(event, false, true)
        }
        size="compact-xs"
        variant="subtle"
      >
        ...
        {renderToolbarAlertBannerContent?.({
          ...
          >
            {enableRowSelection &&
              enableSelectAll &&
              positionToolbarAlertBanner === "head-overlay" && (
                <MTT_SelectCheckbox table={table} />
              )}
            <Stack>
              {alertProps?.children}
              {selectedAlert}
